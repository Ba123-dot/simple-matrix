...
#define SECRET_KEY_SIZE =7*(s^4)+2*s^2*s^2*(s^2+1)/2 // But I foget that where 7*(s^4) comes from?， 当我们不需要S时，可以变成6*(s^4)
*/
WORD Logtable[FIELD] = {
...
int keypair(WORD *sk, WORD *pk)
{
    ...
    // generate B and store B to sk // sk 从equation*equation+1 开始放 B的 数据
               ...
               int ciphertextlen)
{
    ...
    // next find F^{-1}// 为什么看上去这里的coeff 要记录更多的data
    for (k = 0; k < ACOL; k++)
    {
        // 计算E1
        for (i = 0; i < BCOL; i++)
